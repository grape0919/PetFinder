...
public class BluetoothHelper {
    ...
    private int mState; // 상태를 나타내는 상태 변수
    private static final int STATE_NONE = 0; // we're doing nothing
    private static final int STATE_LISTEN = 1; // now listening for incoming connections
    private static final int STATE_CONNECTING = 2; // now initiating an outgoing connection
    private static final int STATE_CONNECTED = 3; // now connected to a remote device

    BluetoothHelper(Activity ac) {
        mActivity = ac;
        btAdapter = BluetoothAdapter.getDefaultAdapter();
        if (!getBtStatus()) {
            Toast.makeText(mActivity.getApplicationContext(), "Device does not support Bluetooth", Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(mActivity.getApplicationContext(), "Device does support Bluetooth", Toast.LENGTH_LONG).show();
            if (!isEnabled()) {
                Log.d(TAG, "getBTstatus()");
     ...
     * 블루투스 지원 여부 확인
     **/
    private boolean getBtStatus() {
        Log.i(TAG, "@BluetoothHelper.getBTstatus() : 블루투스 지원 여부 확인");
        if (mBTAdapter == null) {
    ...
    private boolean isEnabled() {
        Log.i(TAG, "@BluetoothHelper.getBTstatus() : 블루투스 사용 확인");
